...
{    
      ...
      octomap::OcTree octo_tree(0.25);
      octo_tree.readBinary("/home/joeyyu/catkin_Btraj_joeyyu/src/power_plant.bt");
      for(octomap::OcTree::leaf_iterator it = octo_tree.begin_leafs(); it != octo_tree.end_leafs();++it)
      {
          ...
          {
              pt_random.x = static_cast<float>(it.getX());
              pt_random.y = static_cast<float>(it.getY());
              pt_random.z = static_cast<float>(it.getZ());
